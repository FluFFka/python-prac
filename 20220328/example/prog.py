...
class repl(cmd.Cmd):
    ...
    def do_add(self, arg):
        ...
        if (
            ...
            print(
                "Command pattern: add monster name <monster_name> hp <number_of_health_points> coords <X> <Y>"
            )
    ...
    def do_move(self, arg):
        ...
        if len(args) >= 1 and args[0] in ("up", "down", "left", "right"):
            ...
            print(
                "Command pattern: move <direction>\nDirection must be up, down, left or right"
            )
            ...
            for i, monster in enumerate(field[pl.x][pl.y]):
                if monster.name == args[0]:
                    if monster.hp - 10 > 0:
                        ...
                        print(
                            f"{monster.name} lost 10 hp, now has {field[pl.x][pl.y][i].hp} hp"
                        )
                    else:
